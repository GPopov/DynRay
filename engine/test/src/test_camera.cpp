...
TEST_CASE("Raster Space to World Space", "[camera]")
{
   ...
   SECTION("Camera at origin projects center of screen to 0,0")
   {
      ...
      glm::vec4 worldSpacePoint = camera.RasterToWorldSpace(point, 640, 480);
...
TEST_CASE("Generate Primary ray direction" "[camera]")
{
   ...
   SECTION("Camera at origin, oriented along -z axis, top left pixel ray")
   {
      ...
      glm::vec4 expectedDir = glm::normalize(glm::vec4(-ASPECT, 1.f, -1.f, 0.f));
   ...
   {
      glm::vec4 rayDir = camera.GeneratePrimaryRayDirection(WIDTH, HEIGHT, WIDTH-1, HEIGHT-1);
   ...
   SECTION("Camera rotated, has same directions as the directions of the axis aligned camera, transformed by the ToWorld matrix")
   {
      ...
      glm::vec4 rayDir = camera.GeneratePrimaryRayDirection(WIDTH, HEIGHT, 0, 0);
      glm::vec4 expectedDir = glm::normalize(camera.m_ViewMatrix * glm::vec4(-ASPECT, 1.f, -1.f, 0.f));
